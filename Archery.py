...
archer_img = pygame.image.load(
    'Images/ArcherStrip.png')  # Load your sprite here
...
arrow_count_text = GameText(
    f'Arrows Left: {arrow.num_arrows}', 36, BLACK, (300, 20))
game_over_text = GameText('Game Over', 36, RED,
                          (width // 2 - 50, height // 2 - 20))
play_again_text = GameText('Play Again? (y/n)', 36,
                           GREEN, (width // 2 - 50, height // 2 + 10))
game_over = False
...
def handle_arrow_firing():
    ...
    bar_position_ratio = (
        button_midpoint - moving_bar.bar_x) / moving_bar.bar_width
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE and\
                    arrow.arrow_state == ArrowStates.READY and\
                    arrow.num_arrows > 0:
                archer.current_frame = 0
    ...
    if game_over:
        continue  # Skip the rest of the game loop if game is over
    ...
    distance_to_target = ((arrow_x - target_x)**2 +
                          (arrow_y - target_y)**2)**0.5
