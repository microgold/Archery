...
class Archer(pygame.sprite.Sprite):
    def __init__(self, image, position):
        ...
        self.frames = self.get_frames(
            self.image, self.frame_width, self.frame_height, self.num_frames)
...
class Target(pygame.sprite.Sprite):
    def __init__(self, position, radius):
        ...
        for i, color in enumerate(colors):
            pygame.draw.circle(
                self.image, color, (self.radius, self.radius), self.radius - i * 10)
        # draw a black line around the outer edge of the target
        pygame.draw.circle(self.image, BLACK, (self.radius,
                           self.radius), self.radius, 1)
        # draw a black circle outline inside the smallest circle of the target
        pygame.draw.circle(self.image, BLACK, (self.radius,
                           self.radius), self.radius * 0.1, 1)
...
class MovingBar (pygame.sprite.Sprite):
    ...
    #     if self.button_increment > cosine_period - 1:
    #         self.button_increment = cosine_period - 1
    #         self.button_direction = -1

    #     self.button_x = 173.628 * \
    #         math.cos((2*math.pi/cosine_period) * self.button_increment +
    #                  math.pi) + 173.628 + 1

    #     print("button x ", self.button_x)
    #     self.button_x = (self.button_x *
    #                      float(self.bar_width - self.button_width) /
    #                      self.button_position[-1]) + self.bar_x

    # update by position array
    def update(self):
        ...
        self.button_x = int(
            self.bar_x + float(self.button_position[self.button_increment]) * float(self.bar_width - self.button_width) / self.button_position[-1])  # 385.0)

    def draw(self, screen):
        # Draw the moving bar
        pygame.draw.rect(screen, BLUE, (self.bar_x, self.bar_y,
                         self.bar_width, self.bar_height))
        pygame.draw.rect(screen, GREEN, (self.button_x,
